...
# Resolve the directory that contains the running script / executable.
...
# On Linux/Pi, nrsc5 is a system binary (install via apt or build from source).
# On Windows, look for nrsc5.exe next to the script (kept for dev compatibility).
NRSC5_EXE = os.path.join(_APP_DIR, "nrsc5.exe") if sys.platform == 'win32' else "nrsc5"

# Pi-friendly monospace font (Consolas is Windows-only; DejaVu Mono ships with Pi OS)
MONO_FONT = "DejaVu Sans Mono, Monospace" if sys.platform != 'win32' else "Consolas"
...
class NRSC5Manager(QThread):
    ...
    def __init__(self, frequency, program_index, device_index=None, rtlsdr_index=0):
        super().__init__()
        self.freq = frequency
        self.prog = program_index
        self.rtlsdr_index = rtlsdr_index   # RTL-SDR dongle index (Pi may have multiple)
        self.process = None
        ...
        self._station_logo_path = None
        self._art_expiry = None

    def _wait_for_file(self, path, retries=10, delay=0.3):
        """Poll for a file to appear and have non-zero size."""
    ...
    def _emit_newest_image(self):
        ...
        try:
            candidates = (glob.glob(os.path.join(AAS_PATH, "*.jpg")) +
                          glob.glob(os.path.join(AAS_PATH, "*.jpeg")) +
                          glob.glob(os.path.join(AAS_PATH, "*.png")))
            if candidates:
        ...
        for line in iter(self.process.stderr.readline, b''):
            ...
            if "Data component:" in decoded and getattr(self, "_sig_current_program", None) is not None:
                try:
                    ...
                    port_decimal = int(port_str_raw, 10)
            ...
            if "LOT file:" in decoded:
                try:
                    ...
                    if port_str and fname:
                        ...
                        if port_val in (art_port, logo_port):
    ...
    def run(self):
        # Build nrsc5 command.
        # On Linux/Pi, use '-d <index>' to select the RTL-SDR dongle.
        # On Windows, keep original flag style.
        if sys.platform == 'win32':
            cmd = [
                NRSC5_EXE,
                "--dump-aas-files", AAS_PATH,
                "-o", "-",
                str(self.freq),
                str(self.prog)
            ]
            ...
            cmd = [
                NRSC5_EXE,
                "--dump-aas-files", AAS_PATH,
                "-d", str(self.rtlsdr_index),   # RTL-SDR device index
                "-o", "-",
                str(self.freq),
                str(self.prog)
            ]
        ...
        # Windows-only flag to suppress console window
        if sys.platform == 'win32':
    ...
    def stop(self):
        ...
        if self.process:
            if sys.platform != 'win32':
                # On Linux/Pi, send SIGTERM first, then SIGKILL if needed
                try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- RTL-SDR Device Selection (Pi-specific) ---
        rtl_layout = QHBoxLayout()
        rtl_layout.addWidget(QLabel("RTL-SDR:"))
        self.combo_rtlsdr = QComboBox()
        self.combo_rtlsdr.setStyleSheet("background: #111; color: #0F0; font-size: 11pt; border: 1px solid #333;")
        self._rtlsdr_devices = self._enumerate_rtlsdr_devices()
        if self._rtlsdr_devices:
            for idx, name in self._rtlsdr_devices:
                self.combo_rtlsdr.addItem(f"[{idx}] {name}")
        else:
            self.combo_rtlsdr.addItem("[0] Default")
            self._rtlsdr_devices = [(0, "Default")]
        rtl_layout.addWidget(self.combo_rtlsdr, stretch=1)
        layout.addLayout(rtl_layout)
        ...
        for lbl in [self.lbl_country, self.lbl_fccid, self.lbl_bitrate, self.lbl_location]:
            lbl.setStyleSheet(f"color: #0A0; font-size: 9pt; font-family: '{MONO_FONT}'; border: none;")
        ...
        self.debug_log.setStyleSheet(
            f"background: #050505; color: #080; font-family: '{MONO_FONT}'; border: 1px solid #111;"
        )
    ...
    # ----------------------------------------------------------------
    # RTL-SDR device enumeration (Linux/Pi)
    # ----------------------------------------------------------------
    def _enumerate_rtlsdr_devices(self):
        """Attempt to list connected RTL-SDR dongles using rtl_test -t."""
        devices = []
        if sys.platform == 'win32':
            # On Windows nrsc5 handles device selection internally; just return index 0
            return [(0, "Default")]
        try:
            ...
            output = result.stderr  # rtl_test writes to stderr
            for line in output.splitlines():
                line = line.strip()
                # Lines look like:  "  0:  Realtek, RTL2838UHIDIR, SN: ..."
            ...
            # rtl_test not available or timed out — fall back to a single default device
            pass
    ...
    def _delete_preset(self):
        ...
        msg.setStyleSheet(
            "QMessageBox { background: #111; color: #FFF; } QLabel { color: #FFF; font-weight: normal; } "
            "QPushButton { background: #222; color: #0F0; border: 1px solid #333; padding: 4px 12px; }"
        )
        reply = msg.exec()
    ...
    def handle_toggle(self):
        if self.worker and self.worker.isRunning():
            ...
            self.combo_rtlsdr.setEnabled(True)
            self.combo_device.setStyleSheet("background: #111; color: #0F0; font-size: 11pt; border: 1px solid #333;")
        else:
            self.kill_ghosts()
            self.purge_assets()
            ...
            self.combo_rtlsdr.setEnabled(False)
    ...
    def kill_ghosts(self):
        """Kill any lingering nrsc5 processes."""
        if sys.platform == 'win32':
            subprocess.run(
            ...
            # Linux / Raspberry Pi — use pkill
            try:
                subprocess.run(["pkill", "-f", "nrsc5"], capture_output=True)
            except Exception:
                pass
    ...
    def start_worker(self):
        # Resolve audio output device index
        combo_idx = self.combo_device.currentIndex()
        if 0 <= combo_idx < len(self._audio_devices):
            dev_index = self._audio_devices[combo_idx][0]
        else:
            dev_index = None

        # Resolve RTL-SDR dongle index
        rtl_idx = self.combo_rtlsdr.currentIndex()
        if 0 <= rtl_idx < len(self._rtlsdr_devices):
            rtlsdr_index = self._rtlsdr_devices[rtl_idx][0]
        else:
            rtlsdr_index = 0

        self.worker = NRSC5Manager(
            self.spin_freq.value(),
            self.combo_chan.currentIndex(),
            device_index=dev_index,
            rtlsdr_index=rtlsdr_index
        )
